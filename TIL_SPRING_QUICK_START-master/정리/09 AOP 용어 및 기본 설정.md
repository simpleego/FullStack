...
```
**개인적 정리**    
클래스마다 사용되는 공통기능인 횡단 관심에서도  
어떠한 패지지 및 클래스 또는 메소드에서는 공통으로 사용할 메소드라도 사용되지 않을 경우가 있다.  
그럴 경우 포인트컷을 이용하여 사용하려는 메소드만을 정의시켜서 사용하게 끔 하는 것이다.  
(클래스를 직접 실행시켜보면 우선 Impl내의 모든 메소드에 반응은 하지만 실질적으로 호출하지는 않는다.)     
       
## 1.3. 어드바이스
**간략히 말하면 AOP 적용시에 사용되는 메소드 즉 AOP 메소드**  
...
## 1.4. 위빙
**간략히 말하면 포인트컷으로 지정된 메소드가 호출될 때 Aop메소드 즉 어드바이스 메소드가 호출되는 과정을 의미**  
...
애스팩트와 어드바이스를 구현하는 어드바이저는 같은 의미의 용어를 나타내는 것이다.   
...
![KakaoTalk_20191115_152018117](https://user-images.githubusercontent.com/50267433/68921289-98ad6280-07bb-11ea-8cd1-786d67cc87c7.jpg)
...
```   
스프링 컨테이너는   
설정 파일에 등록된 ```<tx:advice>```엘리먼트를 해석하여 트랜잭션 기능의 어드바이스 객체를 메모리에 생성한다.  
그리고 ```<tx:advice>```를 설정했느냐에 따라서 생성되는 어드바이스의 기본 동작 방식은 달라진다.   
